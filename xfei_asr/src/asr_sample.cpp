...
void WakeUp(const std_msgs::String::ConstPtr& msg)
{
 ...
 //cout<< msg->data.c_str();
	...
	run_asr("wav/iflytek01.wav", session_begin_params, grammar_id);
   printf("end?\r\n");
...
{	
	int			ret						=	MSP_SUCCESS;
	const char* login_params			=	"appid = 5fba26bf, work_dir = ."; //登录参数,appid与msc库绑定,请勿随意改动     
          ros::init(argc, argv, "xfvoice");
    ros::NodeHandle n;
    ros::Publisher pub = n.advertise<std_msgs::String>("xfcmd", 1000);
    ros::Rate loop_rate(0.1);

    ros::Subscriber sbu = n.subscribe("xfvoc", 1000, WakeUp);   //接收xfvoc指令來開始讀取wav進行識別
    ros::Publisher pub1 = n.advertise<std_msgs::String>("xfwords", 1000);//發送xfwords 為識別結果
